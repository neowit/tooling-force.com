...
trait JsonProperties {
    ...
    def getString(key: String): Option[String] = {
        jsonProps.getPropertyOption(key)
    }
    def setString(key: String, value: String) {
        jsonProps.setProperty(key, value)
    }
    ...
    def getKeyByValue(fName: String, fVal: String):Option[String] = {
        ...
        jsonProps.keySet().toArray.find(key => Some(fVal) == getData(key.asInstanceOf[String]).get(fName)) match {
